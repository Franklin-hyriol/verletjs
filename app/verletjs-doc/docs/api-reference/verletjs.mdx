...
<Tabs groupId="lang">
  ...
  *Note: The `deltaTime` parameter from previous versions is now ignored.*
  ...
  A function to take over rendering. When provided, the default renderer is disabled. See the [Rendering](/rendering#custom-rendering) page for details.
  ...
  These props allow you to respond to user input. They are functions that receive the React `SyntheticEvent` and the array of simulation `composites`. See the [Interaction](/interaction) page for examples.
